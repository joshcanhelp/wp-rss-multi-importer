...
define( 'WP_RSS_MULTI_VERSION', 3.12 );
...
do_action('rssmi_load_more_data',$numPages,$currentPage,$nextPost,WP_RSS_MULTI_IMAGES,$pag); 
	...
	//pagination controls at bottom
		...
		//	if($currentPage>0 && $q==0){$readable .='<li class="prev"><a href="http://'.$currentURL.'pg=' . ($currentPage-1) . '">Prev</a></li>';}
			if($currentPage>0 && $q==0){$readable .='<li class="prev"><a href="http://'.$currentURL.'pg=' . ($currentPage-1) . '">'.__("Prev", 'wp-rss-multi-importer').'</a></li>';}
		...
		if( $q==$numPages-1 && $currentPage<>$numPages-1){$readable .='<li class="next"><a href="http://'.$currentURL.'pg=' . ($currentPage+1) . '">'.__("Next", 'wp-rss-multi-importer').'</a></li>';}
		}
			$readable .='</ul></div>';
